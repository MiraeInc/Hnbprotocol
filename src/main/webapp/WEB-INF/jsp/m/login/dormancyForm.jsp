...
<script>
	...
	// 동의
	function goAgree(){
		var params = $("form[name=dormancyForm]").serialize() ;
		
		$.ajax({
			 url: "${CTX}/ajax/login/dormancySaveAjax.do",
			 data : params,
			 type: "post",	
			 async: false,
			 cache: false,
			 ...
			 success: function(flag){
				 if(Number(flag) >0){
					 var memberNm = "${detail.memberNm }";
					 alert(memberNm+" 회원님의 계정이 휴면상태에서 해제되었습니다.\면역공방 온라인 쇼핑몰에서 더 많은 혜택을 누리세요.");
...
<body>
	<div id="single">
		<header id="header">
			...
			<h2 class="page-title">휴면계정 복구</h2>
		</header>
		<article id="container" class="clearfix">
			<div id="content" class="inner">
				
							
				<h3 class="suspended-account">
					<span class="color-1">${detail.memberNm}</span> 회원님의 계정은<br/> 
					휴면 상태입니다.
				</h3>
			
				<div class="suspended-desc">
					<p class="">
						* 회원님의 계정은 최근 1년 내 로그인 기록이 없어<br/>
						자동으로 휴면 계정으로 전환되었습니다.<br/>
						<br/>
						아래 내용을 확인하시고, 휴면상태를 해제해 주세요.
					</p>
				</div>
				
				<form name="dormancyForm" method="post" onsubmit="return false;">
					<input type="hidden" name="memberIdx" value="${detail.memberIdx}" >
					<input type="hidden" name="memberId" value="${detail.memberId}" >
					<input type="hidden" name="memberNm" value="${detail.memberNm}" >
					<input type="hidden" name="memberPwd" value="${detail.memberPw}" >
					<input type="hidden" name="joinType" value="${detail.joinType}" >
					<input type="hidden" name="snsCd" value="${detail.snsCd}" >
				
					<div class="suspended-terms">
					<div class="term-title">
						이용약관 <a href="#divTermPolicy1" class="term-anchor">[전문보기 클릭]</a>
					</div>
					<div id="divTermPolicy1" class="term-content">
				...
				<div class="suspended-terms">
					<div class="term-title">
						개인정보처리방침 <a href="#divTermPolicy2" class="term-anchor">[전문보기 클릭]</a>
					</div>
					<div id="divTermPolicy2" class="term-content">
				...
				<div class="suspended-button">
					<a href="javascript:" class="btn btn-black" onclick="goAgree();">약관 동의 및 휴면 해제</a>
				</div>

				<script>
					$(function(){
						$('.term-anchor').on('click', function(){
							var $this = $(this);
