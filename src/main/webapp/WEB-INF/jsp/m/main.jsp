...
<body>
	<div class="content comm-main">
		...
		<div class="main-bestpick">
			...
			<div class="product-bestpicks type-mandom">
				<ul>
					<c:forEach var="list" items="${bestList}" varStatus="idx" begin="0" end="2">
					<li class="<c:if test="${list.soldoutYn eq 'Y'}">soldout</c:if> BEST_${list.tabGubun}">
	...
	<c:if test="${list.mandomYn eq '1'}">
		...
		<c:choose>
			...
			<div class="pop-close">
				...
				<a href="javascript:void(0);" onclick="mainClosePop2();" class="btn-close" data-remodal-action="close">닫기</a>
			</div>
		</div>
	</c:if>
</c:forEach>
