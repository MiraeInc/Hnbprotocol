...
$(function(){
	...
	//장바구니 미리보기
	$('.search-cart').on('mouseenter', '.lnk-menu', function(e){
		var $this = $(this);

		$this.closest('li').siblings().find(' .lnk-menu + div').hide();
		$this.next().show();

		$('.search-cart').one('mouseleave', function(){
			$this.next().hide();
		});
	...
	//카테고리
	 $('#cateMenu').on('mouseenter', '.menu-link', function(){
		var $this = $(this);

		if($this.hasClass('link-best')) {
			return false;
		}
		
		$this.parents('#cateMenu').addClass('active');
	}); 
