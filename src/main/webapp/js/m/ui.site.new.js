...
$.fn.gnb = function(){
    ...
    _gnbOpen = function(){
		...
		if(!isOpen){
			...
			$('.mandom-menu').addClass('aside-open');
            //이벤트 중복실행  방지
            $tab.off('click.gnb.close');
            $tab.off('click.gnb.tab');

            //이벤트
            $('.mandom-menu').off('click.gnb.close').one('click.gnb.close', '#btnMenuClose2', function(){
                _gnbClose();
            })
...
//		});
    ...
    _gnbClose = function(){
        if(isOpen){
			isOpen = false;
			$('.mandom-menu').removeClass('aside-open');
			$('body').removeClass('aside-open');
            $gnb.removeClass('active').scrollTop(0);
        }
    }
...
$.fn.productTabs = function(){
	...
	var $obj = $(this);
	var total = $obj.find('.tab-item').length;
	var size = parseInt($obj.attr('data-size'));
	var tabHeight = 60;
	var grp = 1;

	var activeIndex = $obj.find('.tab-item.active').index();

	for(var i=0; i<total; i++){
		if(i%size==0){
			for(var j=0; j<size; j++) {
				$obj.find('.tab-item').eq(j+i).attr('data-group', grp);
			}
			grp++;
		}
	}

	$obj.find('.tab-anchor').each(function(){
		var $this = $(this);
		var $next = $this.next('.sub-tabs');
		var $parent = $this.parent('.tab-item');
		var grp = $this.closest('.tab-item').attr('data-group');

		if($parent.hasClass('active') && $next.length){
			$obj.find('.tab-item[data-group='+grp+']').height(tabHeight+$next.height()-1);
		}
	})
...
$.fn.productList = function(){
	...
	$obj.each(function(){
		...
		$this.find('.item').css('height', temp);
	})
}
...
$.fn.category = function(){
	...
	$obj.find('.cate-item').each(function(){
		var $this = $(this);
		if(!$this.find('.cate-link').next('.sub-list').length){
			$this.addClass('item-nosub');
		}
	})

	$obj.on('click.link', '.cate-link', function(){
		var $this = $(this);
		if(!$this.next('.sub-list').length) return;

		$obj.find('.sub-list').stop().slideUp();
		$this.next('.sub-list').stop().slideToggle();
		$this.closest('.cate-item').toggleClass('active').siblings('.cate-item').removeClass('active');
	})
}
...
$(function(){
	...
	$('#btnMenuBrand').on('click', function(){
		if(!$('.mandom-menu').hasClass('aside-open')){
			$('#btnMenu').trigger('click');
			$("#gnb").scrollTop($('#gnb').find('.aside-category').position().top);
		}
	});

	$('.cate-list').category();

	$('#btnNavAll').on('click', function(e){
		var $this = $(this);
		$this.toggleClass('active');
		$('#divNavAll').toggle();
		$('#divNavAll').off('click').on('click', '.btn-close', function(){
			$(this).closest('#divNavAll').hide();
			$this.removeClass('active');
		})
		return false;
	});

	$('.mandom-menu').on('touchstart', '.menu-anchor', function(){
		var $this = $(this);
		$this.addClass('active');
	});

	$('.mandom-menu').on('touchend', '.menu-anchor', function(){
		var $this = $(this);
		$this.removeClass('active');
	});

	$('#brandList').on('click', '.btn-morelink', function(){
		var $this = $(this);
